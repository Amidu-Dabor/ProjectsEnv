...
API_GATEWAY_URL = "http://localhost:5008/query"
AUTH_SERVICE_URL = "http://localhost:5001/login"

def authenticate(username: str, password: str) -> Tuple[str, str]:
    """Authenticate and return token and status message."""
...
def stream_response(query: str, mode: str, token: str, chat_history: list) -> Generator[Tuple[str, str], None, None]:
    """Stream response from the API Gateway."""
    headers = {}
    if mode == "study":
...
# Custom CSS for a professional, light-themed chat interface
css = """
...
with gr.Blocks(css=css) as demo:
    ...
    with gr.Tabs():
        with gr.Tab("General Academic Enquiries"):
            ...
            send_btn.click(send_general, inputs=[query_input, chat_history_state], outputs=[chatbot, voice_output], stream=True)
            # send_btn.stream(send_general, inputs=[query_input, chat_history_state], outputs=[chatbot, voice_output])
        ...
        with gr.Tab("Study With Me"):
            ...
            login_btn.click(do_login, inputs=[auth_username, auth_password], outputs=[auth_token_state, login_status])
            ...
            study_send_btn.click(send_study, inputs=[study_query_input, chat_history_state, auth_token_state], outputs=[study_chatbot, study_voice_output], stream=True)
