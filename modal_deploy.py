...
# Each service is deployed as a class.
# Note: We remove the env=... parameters. Your service code should load
# necessary configuration from the environment (e.g. using python-dotenv).

@stub.cls(image=auth_service_image)
class AuthService:
...
@stub.cls(image=data_service_image)
class DataService:
    @modal.web_endpoint()
...
@stub.cls(image=general_chat_service_image)
...
@stub.cls(image=study_support_service_image)
...
@stub.cls(image=response_service_image)
...
@stub.cls(image=system_prompt_service_image)
...
@stub.cls(image=training_service_image)
...
@stub.cls(image=api_gateway_image)
class ApiGateway:
    @modal.web_endpoint()
