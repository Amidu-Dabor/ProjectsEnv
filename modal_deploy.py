...
@app.function(image=auth_service_image, env={"PORT": "5001"})
def run_auth_service():
    import subprocess
    subprocess.run(["python", "app.py"], check=True)

@app.function(image=data_service_image, env={"PORT": "5004"})
def run_data_service():
    import subprocess
    subprocess.run(["python", "app.py"], check=True)

@app.function(image=general_chat_service_image, env={"PORT": "5002"})
def run_general_chat_service():
    import subprocess
    subprocess.run(["python", "app.py"], check=True)

@app.function(image=study_support_service_image, env={"PORT": "5003"})
def run_study_support_service():
    import subprocess
    subprocess.run(["python", "app.py"], check=True)

@app.function(image=response_service_image, env={"PORT": "5005"})
def run_response_service():
    import subprocess
    subprocess.run(["python", "app.py"], check=True)

@app.function(image=system_prompt_service_image, env={"PORT": "5006"})
def run_system_prompt_service():
    import subprocess
    subprocess.run(["python", "app.py"], check=True)

@app.function(image=training_service_image, env={"PORT": "5007"})
def run_training_service():
    import subprocess
    subprocess.run(["python", "app.py"], check=True)

# Deploy API gateway with environment variables overriding its endpoints.
@app.function(image=api_gateway_image, env={
    "PORT": "5000",
...
@app.function(image=ui_image, env={
    "PORT": "7860",
