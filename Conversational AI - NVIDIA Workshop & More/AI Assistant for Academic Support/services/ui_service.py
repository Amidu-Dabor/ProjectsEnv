...
def general_chat_ui(system_prompt: str, model: str):
    ...
    def general_chat(message: str, chat_history) -> Generator[str, None, None]:
        ...
        if GLOBAL_RETRIEVER is not None:
            try:
                ...
                docs = GLOBAL_RETRIEVER.invoke(message) # Retrieve relevant documents
        ...
        for attempt in range(max_retries):
            try:
                ...
                # Check if the error is due to API overload (529)
                if e.status_code == 529:
                    if attempt < max_retries - 1:
                        # Clear any partial response before retry
                        final_response = ""
                        ...
                        # Max retries reached
                        error_msg = "The service is currently busy. Please try again in a few moments."
                        if wandb_tracker:
                    ...
                    if wandb_tracker:
                        wandb_tracker.log_error("claude_api_error", str(e), session_id, message)
                    print(error_msg)
                    break
    ...
    # Generate the initial greeting once and store it
    try:
        initial_greeting = generate_initial_greeting_general(system_prompt, model)
    except Exception as e:
        print(f"[Warning] could not generate dynamic greeting with Claude: {e}")
        # Fallback static greeting
        initial_greeting = "Hello and welcome! I'm your AI assistant, here to help you with queries related to USIU. Feel free to ask any questions or seek assistance."
    
    # Create the initial message list with the generated greeting
...
def study_support_ui(system_prompt: str, model: str):
    ...
    def study_support_chat(message: str, chat_history: list, session_id: str) -> Generator[str, None, None]:
        ...
        if current_file_path:
            try:
                ...
                if auto_clear_files_state: # Clear the file if auto-clear is enabled
                    current_file_path = None
        ...
        # ---- Stream from Claude with proper retry mechanism ---------------------------------------
        final_response = ""
        
        max_retries, retry_delay = 5, 3 # Initialize retry parameters
        ...
        for attempt in range(max_retries):
            try:
                ...
                end_time = time.time() # Success - log to Weights & Biases (W&B)
                response_time = end_time - start_time
                
                if wandb_tracker:
                    token_estimate = len(message.split()) + len(final_response.split()) * 1.3 # Estimate token count (rough estimate)
                    if file_text:
                ...
                if e.status_code == 529: # Check if the error is due to API overload (529)
                    if attempt < max_retries - 1:
                        final_response = "" # Clear any partial response before retry
                        time.sleep(retry_delay) # Wait before retrying
                        retry_delay *= 1.5  # Exponential backoff
                        continue
                    else:
                        error_msg = "The service is currently busy. Please try again in a few moments." # Max retries reached
                        if wandb_tracker:
                            wandb_tracker.log_error("claude_overload", str(e), session_id, message)
                        yield error_msg
                        break
    ...
    # Generate the initial greeting once and store it
    try:
        initial_greeting_study = generate_initial_greeting_study(system_prompt, model)
    except Exception as e:
        print(f"[Warning] could not generate dynamic greeting with Claude: {e}")
        # Fallback static greeting
        initial_greeting_study = "Hello and welcome there! I'm your AI study bud, here to assist you with your study journey."
    
    # Create the initial message list with the generated greeting
    initial_messages_study = [{"role": "assistant", "content": initial_greeting_study}]
    ...
    with gr.Blocks(elem_id="study-chat-interface") as interface:
        ...
        chat_interface = gr.ChatInterface(
            ...
            additional_inputs=[session_id_box],  # Pass session_id as additional input
        )
